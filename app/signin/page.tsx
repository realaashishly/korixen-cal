...
const SignupPage: React.FC = () => {
  ...
  // 3. Prevent "Content Flash" - Checks if user is already logged in
  useEffect(() => {
  ...
  // Loading screen to prevent seeing the form while checking session
  if (isPageLoading) {
    return <div className="h-screen w-full flex items-center justify-center bg-[#FAFAFA] dark:bg-zinc-950"><Logo /></div>;
  }
  ...
  return (
    <div className="flex h-screen w-full bg-[#FAFAFA] dark:bg-zinc-950 bg-dot-pattern items-center justify-center p-4">
      <div className="w-full max-w-md bg-white dark:bg-zinc-900 rounded-[32px] p-8 shadow-2xl border border-gray-100 dark:border-zinc-800 animate-in zoom-in-95 duration-500">
        ...
        {/* GOOGLE BUTTON - Logic Fixed: Shows "Continue" until clicked */}
        <button 
          type="button"
          onClick={handleGoogleSignIn}
          disabled={isSocialLoading || isSubmitting}
          className="w-full py-3 rounded-xl border border-gray-200 dark:border-zinc-700 flex items-center justify-center gap-3 hover:bg-gray-50 dark:hover:bg-zinc-800 transition-colors mb-6 group disabled:opacity-50"
        >
          {!isSocialLoading && (
          ...
          <span className="font-bold text-sm text-gray-700 dark:text-gray-300 group-hover:text-black dark:group-hover:text-white transition-colors">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input 
              ...
              className="w-full bg-gray-50 dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded-xl px-4 py-3 font-medium text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-black dark:focus:ring-white/20 transition-all"
        ...
        <p className="text-center mt-6 text-xs text-gray-400">
          Already have an account? <Link href="/signup" className="text-black dark:text-white font-bold cursor-pointer hover:underline">Sign up</Link>
        </p>
      </div>
    </div>
  );
